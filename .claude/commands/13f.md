...
**First-time setup check:** If any command fails with "SEC_CONTACT_EMAIL" error, remind the user:
```
export SEC_CONTACT_EMAIL=your@email.com  # Add to ~/.zshrc
```
...
**Error handling:**
- If "SEC_CONTACT_EMAIL" error → remind user to set the environment variable
- If "Fund not found" → run `13f list-funds` and retry with exact fund name
- If "No data for fund" → run `13f pull --fund "Fund Name"` first
- If command fails → check `13f <command> --help` for correct syntax

**Command patterns:**
- Stock commands use positional args: `13f stock NVDA`
- Fund commands use named options: `13f report --fund "Coatue Management"`
